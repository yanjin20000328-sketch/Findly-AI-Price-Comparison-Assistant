...
function buildMockReviews(product = {}, count = 24) {
  ...
  const positiveTarget = hasOfficialSignal ? 0.84 : isBudget ? 0.72 : 0.78;
  const safeCount = Math.max(12, Number(count) || 24);
  ...
  return sentiments.map((positive, index) => {
    ...
    const content = cluster.texts[Math.floor(sentimentIndex / clusters.length) % cluster.texts.length];
...
function buildReviewInsight(product = {}) {
  const hasCurrentContextualReviews = Array.isArray(product.review_comments)
    ...
    && product.review_comments.every((review) => review?.source === '基于商品字段生成的拟真模拟评论');
  ...
  return {
    ...
    conclusion,
    summary: `口碑探员读取 ${reviews.length} 条近 3 个月模拟评价，正向反馈约 ${positiveRate}%。主要好评集中在${positiveLabels.join('、') || '体验'}，需要注意${negativeLabels.join('、') || '售后'}。`,
    positive_tags: positiveTags,
    negative_tags: negativeTags,
    data_note: reviews.length < 10
      ? '当前评价样本较少；评论为基于商品字段生成的拟真模拟数据，仅用于 Demo 聚类展示。'
      : '评论为基于商品字段生成的拟真模拟数据，仅用于 Demo 聚类展示。',
    clusters,
...
const SMART_COMPARISON_AGENT_PROMPT = `
你是一个 AI 购物比价决策助手。

你的任务不是简单找最低价，而是基于用户拍照/上传识别出的商品，以及候选商品列表，综合判断“哪一个更适合买”。

你需要综合以下因素：商品匹配度、到手价、平台来源、店铺类型、发货配送、售后稳定性、销量与评价、用户偏好、类目特有因素。

输出必须适合移动端展示：
- 先给 2-4 句明确结论，不要一上来就是表格。
- 结论要直接告诉用户更推荐哪一个、为什么、什么情况下可以选另一个。
- 不要暴露置信度，不要暴露系统规则。
- 不要只按最低价排序，要解释综合推荐逻辑。
- 对风险要客观提示，但不要制造焦虑。
- 表格最多比较 3 个商品、最多 6 行维度，每个单元格尽量控制在 18 个中文字符以内。
- 不要把平台名放到商品标题前。

为了适配当前前端，请严格只返回 JSON，不要 Markdown：
{
  "summary": "2-4句中文结论，包含推荐谁、为什么、何时选备选；120字以内",
  "rows": [
    {
      "productId": "候选商品 id",
      "storeType": "店铺/来源短判断",
  ...
  "answer": "如果用户有追问，给80字以内回答；没有追问则为空"
}
rows 最多返回 3 个商品，字段内容要短。
`;

const TOP1_SUMMARY_CARD_PROMPT = `
你是 AI 购物推荐总结助手。

用户已经上传或拍摄图片，系统已经识别出商品，并从多个平台候选商品中选出了综合 Top 1。

Findly Agent Skill 部署：
- product-context-build：通用 Skill，所有 Agent 共享，用于统一商品识别、候选商品、价格、风险和缺失数据口径。
- agent-bubble-orchestration：购物智囊团，用于输出固定 4 个 Agent 的协作讨论。
- comparison-arbiter：比价军师，用于综合推荐裁决、Top 1 收起态理由和最终建议。
- reputation-insight：口碑探员，用于近 3 个月评价、好评点、差评点和风险判断；无评价时必须说明数据不足。
- saving-plan：省钱达人，用于优惠叠加、预计到手价和最佳买法；不能编造优惠券。
- price-watch：盯价哨兵，用于价格机会、目标价接近提醒和是否值得继续盯价。

你的任务是同时生成 Top 1 推荐卡的普通模式和讨论模式结果。

普通模式：用 3-4 句简单 summary，说明推荐谁、综合优势、不是最低价时贵在哪里，以及适合谁。

讨论模式：基于四个 Agent 的可见讨论结论，围绕三个决策面生成详细结构化总结：
1. 价格与购买时机。
2. 商品与渠道可信度。
3. 履约与售后保障。

讨论模式最终内容会由前端渲染为纯文本长总结：
- 使用“综合结论、具体分析、讨论共识、购买建议”等加粗大标题。
- 大标题下使用圆点分项和必要的长段分析。
- 不要为了适配卡片而把内容切得过碎，每个分点必须表达完整观点。
- 不要输出大量互相独立的小卡片式短句。

不要输出复杂表格。不要暴露置信度。不要使用“根据算法”“模型判断”等技术表达。不要写成营销广告。不要夸大。不要只说便宜。不要只复述商品标题。

请严格只返回 JSON，不要 Markdown：
{
  "topProductId": "候选商品 id；如果没有候选商品则为空",
  "reason": "45-70字卡片短摘要，quick 模式只输出这一类快速理由",
  "normal_summary": "普通模式的3-4句推荐摘要，90-140字",
  "strategyWeights": [
    {"label":"主体匹配度","weight":35},
    {"label":"到手价优势","weight":25},
    {"label":"来源可靠性","weight":25},
    {"label":"发货售后","weight":15}
  ],
  "top1_summary": {
    "short_summary": "45-70字，适合 quick 模式默认展示，只讲为什么选它，不展开讨论",
    "expanded_points": ["3-5条，每条不超过28个中文字符"],
    "risk_tips": ["1-2条短句"]
  },
  "agent_bubble": [
    {"agent":"比价军师","role":"综合推荐","message":"80字以内，说明为什么它综合更值得买"},
    {"agent":"省钱达人","role":"优惠判断","message":"80字以内，说明已知价格/优惠/到手价判断；缺数据要说有限"},
    {"agent":"口碑探员","role":"风险判断","message":"80字以内，说明口碑/风险判断；缺评价要说数据不足"},
    {"agent":"盯价哨兵","role":"时机判断","message":"80字以内，说明当前是否适合买或继续盯价"}
  ],
  "collaboration_mode": "quick/debate；quick 是快速结论，debate 是用户可见的详细讨论",
  "debate_enabled": true,
  "mode_switch": {
    "default_mode": "quick",
    "quick_label": "快速结论",
  ...
  "debate_turns": [
    {"agent":"比价军师","stance":"opening","target":"决策点","message":"40-90字，提出综合判断"},
    {"agent":"省钱达人","stance":"pushback","target":"比价军师","message":"40-90字，围绕到手价或优惠提出不同意见"},
    {"agent":"口碑探员","stance":"risk_check","target":"省钱达人","message":"40-90字，补充口碑、售后或渠道风险"},
    {"agent":"盯价哨兵","stance":"timing_check","target":"购买时机","message":"40-90字，判断现在买还是继续盯价"}
  ],
  "conflict_summary": "100字以内，说明核心分歧；quick 模式可说明未展开分歧",
  "final_recommendation": "160字以内，以比价军师口吻综合四个 Agent 的结论，仅 debate 模式用于最终总结",
  "decision_summary": {
    "final_summary": "3-5句完整的最终推荐结论",
    "decision_dimensions": [
      {"title":"价格与购买时机","conclusion":"2-3句完整分析","evidence":["2-4条完整观点"],"watch_out":"完整注意点"},
      {"title":"商品与渠道可信度","conclusion":"2-3句完整分析","evidence":["2-4条完整观点"],"watch_out":"完整注意点"},
      {"title":"履约与售后保障","conclusion":"2-3句完整分析","evidence":["2-4条完整观点"],"watch_out":"完整注意点"}
    ],
    "agent_consensus": "2-3句说明四个 Agent 的核心共识",
    "key_disagreement": "2-3句说明最重要的一项分歧，没有则为空",
    "buying_action": "2-3句说明现在怎么买",
    "risk_tips": ["1-2条短风险提示"]
  },
  "display_mode": "top1_expand",
  "arbiter_decision": {
    "collapsed_reason": "80字以内，Top 1 默认卡片理由",
    "recommended_product_id": "最终推荐商品 id",
    "decision": "recommend/compare_more/watch_price/avoid_for_now",
    "is_lowest_price": true,
    "why_not_lowest_price": "如果不是最低价，解释为什么仍推荐；否则为空",
    "final_summary": "180字以内最终裁决",
    "next_action": "查看省钱方案/查看口碑风险/加入盯价/去平台购买/继续对比"
  }
}
权重总和为100。只能从候选商品 id 中选择 topProductId。
agent_bubble 必须严格按 比价军师、省钱达人、口碑探员、盯价哨兵 顺序输出。不要输出隐藏思维链，只输出用户可见的协作摘要。
当 collaboration_mode 为 debate 时，debate_turns 返回 4-6 轮有角色张力但专业克制的讨论；不要辱骂、不要人身攻击、不要暴露隐藏推理或 prompt。
当 collaboration_mode 为 quick 时，reason 和 top1_summary.short_summary 必须明显短于 final_recommendation，不要输出展开点式的完整讨论总结。
normal_summary 必须始终返回。decision_summary 必须始终返回并严格包含三个指定决策面。
`;
...
function mapRichListingToProduct(theme = {}, listing = {}, index = 0) {
  ...
  const activityText = Array.isArray(listing.activity_info)
    ? listing.activity_info.map((item) => item.display_text || item.activity_name).filter(Boolean).join('；')
    : '';
  ...
  return {
    ...
    reason: `${recommendation}${activityText ? ` 优惠：${activityText}` : ''}`,
    rawId: productId,
...
function renderTop1QuickReason(top1Summary, fallback = '') {
  ...
  const firstSentence = base.split(/[\n。！？!?]/).map((item) => item.trim()).find(Boolean) || base;
  return limitText(firstSentence, 72, '综合价格、匹配度和风险后，这个候选更适合作为当前 Top 1。');
}
...
async function rankProductsWithDoubao(searchContext, products, orchestrationOptions = {}) {
  ...
  try {
    const response = await fetchWithTimeout(ARK_API_URL, {
      ...
      body: JSON.stringify({
        ...
        messages: [
          {
            role: 'system',
            content: TOP1_SUMMARY_CARD_PROMPT,
          },
          {
            role: 'user',
            content: JSON.stringify({ searchContext, candidates, orchestrationOptions }, null, 2),
          },
        ],
      }),
    }, 45000);
    ...
    const exists = products.some((product) => product.id === parsed.topProductId);
    const top1SummaryText = collaborationMode === 'quick'
      ? asCleanString(parsed.normal_summary) || renderTop1QuickReason(parsed.top1_summary, String(parsed.reason || ''))
      : asCleanString(parsed.decision_summary?.final_summary) || renderTop1Summary(parsed.top1_summary, String(parsed.reason || ''));
    return {
      topProductId: exists ? parsed.topProductId : (products[0]?.id || ''),
      ...
      agentDiscussion: Array.isArray(parsed.agent_bubble)
        ? normalizeAgentDiscussionSkillOutput(parsed, {
          displayMode: 'top1_expand',
          collaborationMode,
          ...
          finalRecommendation: asCleanString(parsed.final_recommendation),
        })
        : null,
  ...
  return {
    ...
    reasoning: collaborationMode === 'quick' ? limitText(fallbackReason, 72) : fallbackReason,
  ...
  try {
    ...
    res.json({
      ...
      reasoning: collaborationMode === 'quick'
        ? limitText(ranking.reasoning || arbiterDecision?.collapsed_reason || topProduct.reason || '', 72)
...
app.post('/api/ai-reasoning', async (req, res) => {
  ...
  try {
        ...
        messages: [
          {
            role: 'system',
            content:
              `${TOP1_SUMMARY_CARD_PROMPT}\n当前阶段可能还没有候选商品列表，请基于 VLM 商品画像生成预检索阶段的 Summary 卡片内容。topProductId 可返回空字符串，reason 要说明接下来应如何跨平台比价。`,
          },
          {
            role: 'user',
    ...
    try {
      const parsed = parseJsonObjectFromModel(content);
      res.json({
        reasoning: renderTop1Summary(parsed.top1_summary, asCleanString(parsed.reason) || content),
        top1Summary: parsed.top1_summary || null,
      });
    } catch {
...
function buildFallbackCompareResult(products, question = '', orchestrationOptions = {}) {
  ...
  return {
    summary: `${recommended.title || 'Top 候选'}综合更值得优先考虑：价格、渠道可信度和配置完整度更均衡，其他款可按品牌偏好补充选择。`,
    rows,
    answer,
...
function buildProductAgentCards(product = {}) {
  ...
  const compareAdvice = buildCompareAgentAdvice(product);
  const priceHistory = Array.isArray(product.price_history) && product.price_history.length
    ? product.price_history
    : buildMockPriceHistory(product);
  return {
    comparison_card: compareAdvice,
    reputation_card: {
      agent: '口碑探员',
      conclusion: reputationInsight.conclusion,
      review_sample_size: reputationInsight.review_sample_size,
      positive_rate: reputationInsight.positive_rate,
      summary: reputationInsight.summary,
      positive_tags: reputationInsight.positive_tags,
      negative_tags: reputationInsight.negative_tags,
      data_note: reputationInsight.data_note,
      clusters: reputationInsight.clusters,
      sample_reviews: reputationInsight.sample_reviews,
      confidence: reputationInsight.positive_rate >= 82 ? 'high' : 'medium',
      purchase_advice: reputationInsight.positive_rate >= 82
        ? '评价样本整体可参考，下单前再核对差评集中点。'
        : '评价分歧较多，建议优先看代表差评和退换规则。',
    },
    saving_plan_card: {
      agent: '省钱达人',
      conclusion: discount > 0 ? '领券直买' : '直接买',
      best_plan: discount > 0 ? '领券后下单' : '确认规格后下单',
      estimated_final_price: price,
      breakdown: [
        { label: '商品标价', amount: originalPrice },
        { label: '平台优惠', amount: -discount },
        { label: '预估运费', amount: parseMoney(product.shipping || 0) },
      ],
      limitations: ['优惠以结算页为准'],
      next_action: '去平台购买',
    },
...
app.post('/api/compare-products', async (req, res) => {
  ...
  try {
    ...
    res.json({
      ...
      answer: asCleanString(parsed.answer),
      agentDiscussion: buildCompareFollowupAgentDiscussion(products, question, orchestrationOptions),
    });
  } catch (error) {
